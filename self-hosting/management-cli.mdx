...
| `nixopus report` | Print a paste-friendly support bundle (redacted `.env`, recent logs) — [details below](#support-report) |
...
Use **`sudo nixopus report`** when you need to share diagnostics (forum, GitHub, support). It prints:

- The last part of **`/opt/nixopus/install.log`** (if present)
- OS / kernel (`uname`, `/etc/os-release`)
- A short `docker version` summary
- `docker compose ps -a`
- The last 120 lines of **all** service logs
- A **redacted** copy of `.env` (passwords, database/redis URLs, API keys replaced with `<redacted>`)
...
Raw **`install.log`** may still contain secrets that appeared on the terminal during install — prefer `nixopus report` when possible, or edit **`install.log`** before sending.
